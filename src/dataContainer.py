...
def askStream(pth_in: Path, stream: str, message: str) -> O[str]:
    strms = ffprobe(pth_in=pth_in,
                    streams=stream[0],
                    entries="stream=index",
                    frmt="csv=p=0").strip().split("\n")
    if len(strms) > 1:
                ...
                if Opt.aud_strm in AUD_LANGS:
                    ...
                    if not Opt.aud_strm in self.streams:
                        ...
                        ans = askStream(pth_in=pth_in,
                                        stream="audio",
                                        message=f"The specified language couldn't be found. Please select the {AUD_LANGS.get(Opt.aud_strm)} audio stream")
                        if ans:
            ...
            if self.streams:
                ...
                if Opt.sub_strm in SUB_LANGS:
                    ...
                    if not Opt.sub_strm in self.streams:
                        ...
                        ans = askStream(pth_in=pth_in,
                                        stream="subtitle",
                                        message=f"The specified language couldn't be found. Please select the {SUB_LANGS.get(Opt.sub_strm)} subtitle stream")
                        if ans:
